...
/**
 * @param nums 前面计算得到的nums数组
 * @param narray table里的array和hash表中整数的大小
 * @return 返回重新分配后array中元素的
 */
static int computesizes(int nums[], int *narray) {
	int i;
	int twotoi;  /* 2^i */
	int a = 0;  /* number of elements smaller than 2^i */
	int na = 0;  /* number of elements to go to arr
    //重新计算后数组的大小ay part */
 ...
 * @param L
 * @param t
 * @param nasize 数组中元素的个数
 * @param nhsize  hash表中元素的个数
 */
...
static void rehash(lua_State *L, Table *t, const TValue *ek) {
	int nasize, na;
	// nums中存放的是key在2^(i-1), 2^i之间的元素数量, 2^i指的是数组的下标。
	int nums[MAXBITS + 1];  /* nums[i] = number of keys between 2^(i-1) and 2^i */
	int i;
	int totaluse;  //totaluse表示包括array和hash所有的key的数量
...
/**
 * 插入一个key到hash table；
 * 所有的key都是通过node数组中的元素表达，newKey()返回会一个value给调用者设置值。
 * mainPosition是一个集合，这个集合的Node通过next链接成一个链表，并且这个链表的第一个tNode nodeS 一定是mainPosition所在的位置，即nodeS = mainposition(t, nodeS.i_key.tvk)
 * -------
 * mainPos1| -> Node -> Node
 * -------|
 * .......
...
static TValue *newkey(lua_State *L, Table *t, const TValue *key) {
	...
	if (!ttisnil(gval(mp)) || mp == dummynode) {
		...
		/**
		 * mp是key计算出来的mainPosition的Node:node', othern是node'的key中的mainPosition的Node
		 * 通过比较othern 和mp:
		 * 1.二者不相等，说明othern不属于当前这个mainPosition， 是一个colliding的node.
		 * 2.如果二者相等，说明属于同一个mainPosition，放入mainPosition后即可。
		*/
		othern = mainposition(t, key2tval(mp));
		if (othern != mp) {  /* is colliding node out of its main position? */
			/* yes; move colliding node into free position
			othern指向的next链表一定会指向mp？
			是的，mp的Node的mainposition和othern出的node是同一个main_position, 因此othern的main_position链表中必然存在mp.
			*/
...
static int unbound_search(Table *t, unsigned int j) {
	...
	//找到一个j，使得luaH_getnum(t,j)是空，即找到二分查找的右边界
