 ...
 * mainPosition是一个集合，这个集合的Node通过next链接成一个链表，并且这个链表的第一个Node nodeS 一定是mainPosition所在的位置，即nodeS = mainposition(t, nodeS.i_key.tvk)
 * @param L
 * @param t
 * @param key
 * @return
    ...
    if (!ttisnil(gval(mp)) || mp == dummynode) {
        ...
        */
        ...
        if (othern != mp) {  /* is colliding node out of its main position? */
            ...
            是的，mp的Node的mainposition和othern出的node是同一个main_position, 但是由于被占用了，导致被放在了mp.
            */
